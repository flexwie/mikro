...
export class Mikro {
    ...
    constructor(name: string, opts: IMikroOptions) {
        ...
        this.events = new RedisEventStream(opts && opts.redis ? opts.redis : {})
            ...
            transports: [
                new logger.transports.Console({
                    format: logger.format.combine(
                        ...
                        logger.format.printf(info => `${info.timestamp} ${info.level}: ${info.message}`+(info.splat!==undefined?`${info.splat}`:" ")) 
                    )
                })
            ]
        })
